 ...
 *
 */
public class DokunusGoruntuleme extends android.support.v7.widget.AppCompatImageView {

	public PointF lastPointMark = null;
	...
	// rotation.
	//
	public float matchViewWidth, matchViewHeight, prevMatchViewWidth, prevMatchViewHeight;
	...
	public DokunusGoruntuleme(Context context) {
		super(context);
		sharedConstructing(context);
	}

	public DokunusGoruntuleme(Context context, AttributeSet attrs) {
		super(context, attrs);
		sharedConstructing(context);
	}

	public DokunusGoruntuleme(Context context, AttributeSet attrs, int defStyle) {
	...
	public void setZoom(DokunusGoruntuleme img) {
	...
	private void fitImageToView() {
		...
		// Log.i("COMPA", "ScaleX: " + scaleX + " W: " + viewWidth + " DW: " +
		// drawableWidth);
		// Log.i("COMPA", "ScaleY: " + scaleY + " H: " + viewHeight + " DH: " +
		// drawableHeight);
		...
		if (!isZoomed() && !imageRenderedAtLeastOnce) {
			...
			// Log.i("COMPA", "PAW: " + prevActualWidth + " pW: " +
			// prevMatchViewWidth + " NS: " + normalizedScale);
			// Log.i("COMPA", "PAH: " + prevActualHeight + " pH: " +
			// prevMatchViewHeight + " NS: " + normalizedScale);
			//
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if (state == State.NONE || state == State.DRAG || state == State.FLING) {
				...
				case MotionEvent.ACTION_POINTER_UP:
					...
					float imageX = (curr.x - values[2]) / getCurrentZoom();
					float imageY = (curr.y - values[5]) / getCurrentZoom();
					lastPointMark = null;
					if (state == State.DRAG && imageX < getImageWidth() && imageY < getImageHeight() && imageX >= 0
							&& imageY >= 0) {
						lastPointMark = new PointF();
						lastPointMark.x = imageX;
						lastPointMark.y = imageY;
					}
					setState(State.NONE);
					break;
				}
